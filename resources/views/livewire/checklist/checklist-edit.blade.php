<div>
  <x-form.container title="Edit Checklist" description="Update the employee or replace the file if needed.">
    <form wire:submit.prevent="update" class="grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <x-form.field class="md:col-span-6" title="Current File">
        @if ($checklist->filename)
          <a class="link"
             href="{{ \Illuminate\Support\Facades\Storage::url($checklist->filename) }}"
             target="_blank">
      ...
      <x-form.field class="md:col-span-6" title="Replace File (optional)" for="file">
        <div
          ...
          <input id="file" name="file" type="file" wire:model="file" accept=".xlsx,.xls,.csv"
      ...
      <div class="md:col-span-12 flex flex-col sm:flex-row gap-3">
        ...
        <a class="btn btn-ghost"
           wire:navigate
           href="{{ route('checklist.show', $checklist) }}">
          Cancel
        </a>
      </div>
    </form>
