...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="card-body flex flex-row items-center gap-4">
            <div class="p-3 rounded-full bg-yellow-500/20 text-yellow-600">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6l4 2m-4 6a9 9 0 100-18 9 9 0 000 18z" />
                </svg>
            </div>
            <div>
    ...
    <div class="card bg-info/50 shadow-md border border-blue-300">
        <div class="card-body flex flex-row items-center gap-4">
            <div class="p-3 rounded-full bg-blue-300/20 text-blue-600">
                <!-- Users Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-4-4h-1m-6 6h6m-6 0v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2h5" />
    ...
    <!-- Total Zones -->
    <div class="card bg-secondary/50 shadow-md border border-purple-300">
        <div class="card-body flex flex-row items-center gap-4">
            <div class="p-3 rounded-full bg-purple-300/20 text-purple-600">
                <!-- Map Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A2 2 0 013 15.382V5.618a2 2 0 011.553-1.947L9 2m0 0l6 2m-6-2v18m6-16l5.447 2.724A2 2 0 0121 8.618v9.764a2 2 0 01-1.553 1.947L15 22m0 0V4" />
                </svg>
            </div>
            <div>
                <h2 class="text-sm text-black">Zones</h2>
                <p class="text-2xl font-bold text-black">{{ $zoneCount }}</p>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                    ...
                    <tbody>
                        @forelse ($recentChecklists as $a=> $chk)
                            <tr>
                                ...
                                <td>{{ $chk->employee->user->fullname ?? '—' }}</td>
                                <td>{{$chk->status}}</td>
                                <td>{{ $chk->created_at->format('Y-m-d H:i') }}</td>
                            

                                <td class="p-4">
                                    <a wire:navigate href="{{ route('hr.show', $chk->id) }}" class="btn btn-ghost btn-xs"
                                        title="View">
    ...
    <div class="card bg-base-100 shadow-md border border-base-300">
        <div class="card-body">
            <h2 class="card-title text-base mb-4">Top Managers by Uploads</h2>
            <div class="overflow-x-auto">
                <table class="table table-sm w-full">
                    <thead class="text-xs uppercase bg-base-200 text-base-content/60">
                        <tr>
                    ...
                    <tbody>
                        @forelse ($topManagers as $i => $mgrEntry)
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                <td>{{ $mgrEntry->user->first_name }} {{ $mgrEntry->user->last_name }}</td>
                                <td class="text-right font-semibold">{{ $mgrEntry->uploads_count }}</td>
                            </tr>
                        @empty
                            <tr>
